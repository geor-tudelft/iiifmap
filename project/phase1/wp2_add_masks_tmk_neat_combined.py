...
report_path = r"E:\TU_Delft\Synthesis\iiifmap\project\phase1\results_wp2\tmk_neat_combined\report.txt"  # path to write the report
r = open(report_path, 'w')
r.close()
files_directory = r"E:\TU_Delft\Synthesis\iiifmap\project\resources\tmk_neat_combined"  # path with the map sheets
files_names = os.listdir(files_directory)
total = len(files_names)
i = 0  # Counter for progress
for file_name in files_names:
    file = os.path.join(files_directory, file_name)
    # print(file)
    i += 1
    ...
    with open(file, "r") as f:
        ...
        try:
            corners = mask_generator.corner_detection(mapsheet, plot=False)
            mask = RectangleMask.from_corners(*corners)
            mapsheet.set_mask(mask)
            with open(rf"E:\TU_Delft\Synthesis\iiifmap\project\phase1\results_wp2\tmk_neat_combined\{file_name}", 'w') as ap:  # path to write the annotation pages
                ap.write(mapsheet.to_annotationpage())
        except:
            with open(report_path, "a") as report:
                report.write(f"Could not find mask for {file_name}\n")
